...
proto_library(
    name = "flight_proto",
    srcs = ["format/Flight.proto"],
    strip_import_prefix = "format",
    deps = ["@com_google_protobuf//:timestamp_proto"],
)

cpp_grpc_compile(
    name = "flight_grpc_proto_cc",
    prefix_path = "../cpp/src/arrow",
    protos = [":flight_proto"],
)

filegroup(
    name = "flight_grpc_proto_cc_files",
    srcs = [
        ":flight_grpc_proto_cc",
    ],
)

genrule(
    name = "flight_grpc_proto_cc_files_copy",
    srcs = [":flight_grpc_proto_cc_files"],
    outs = [
        "arrow/flight/Flight.grpc.pb.cc",
        "arrow/flight/Flight.grpc.pb.h",
        "arrow/flight/Flight.pb.cc",
        "arrow/flight/Flight.pb.h",
    ],
    cmd = "cp $(locations :flight_grpc_proto_cc_files) $(@D)/arrow/flight",
)
