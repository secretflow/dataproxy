...
class DataProxyStream::Impl {
 public:
  ...
  std::unique_ptr<SimpleStreamWriter> GetWriter(proto::UploadInfo& info) {
    auto content_type = proto::ContentType::Table;
    auto file_format = proto::FileFormat::CSV;
    if (info.type() != "table") {
      content_type = proto::ContentType::RAW;
    ...
    try {
      // 1. 从dm获取dp信息
      auto any = BuildUploadAny(info, content_type);
...
std::unique_ptr<DataProxyStreamReader> DataProxyStream::GetReader(
  ...
  // TODO:
  // proto::ContentType::Table暂时固定，因为在read时dp的odps没有用到这个字段
  // 后续DataProxy会在CommandDomainDataQuery会调整该参数
  auto any = BuildDownloadAny(info, proto::ContentType::Table);
...
void DataProxyStream::DeleteDomainData(const proto::UploadInfo& info) {
  ...
  SPDLOG_WARN("stream write error. upload_info:{}", info.SerializeAsString());
}
