...
import org.secretflow.dataproxy.manager.connector.rdbms.adaptor.consumer.CompositeJdbcConsumer;
import org.secretflow.dataproxy.manager.connector.rdbms.adaptor.consumer.JdbcConsumer;
    ...
    private void initialize(VectorSchemaRoot root) throws SQLException {
        for (int i = 1; i <= consumers.length; i++) {
            ...
            consumers[i - 1] = JdbcToArrowUtils.getConsumer(
                arrowType, i, JdbcToArrowUtils.isColumnNullable(resultSet.getMetaData(), i, columnFieldInfo), root.getVector(i - 1), config);
        }
    }
